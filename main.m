...
while(iter<100)
    ...
    countErrors = 0;
    countSuccesses = 0;
    PESO = [5 12 1 5 6 12 21]; %favor escolher valores maiores ou iguais a 1. 
    ...
    title('Evolucao do fitness ao longo das gerações');
    xlabel('Geração');
    ylabel('Fitness')
 ...
 function y = funcao_fitness(vars) 
    ...
    function y = funcao_objetivo(vars)
        ...
        norm_z(2) = z(2)/(3*3.6904333333337e+03);                            %Q - MAXIMIZE
        norm_z(3) = z(3)/(3*0.1201348351);                                   %Fp - MAXIMIZE
        norm_z(4) = ((3*(z(4)+1e10))/(1e10+1.34463927383e+09))^-1;           %DN -  MINIMIZE
        norm_z(5) = ((3*(z(5)+2e-07))/(2e-07+9.546694853150e-08))^-1;        %Cav_loss - MINIMIZE
        norm_z(6) = ((3*(z(6)+1))/(1+0.89104208416836))^-1;                  %Dist - MINIMIZE
        norm_z(7) = ((3*(z(7)+1))/(1+0.7139))^-1;                            %Reflec - MINIMIZE
        ...
        if(erro)
            ...
            %apesar do individuo não ser descartado a partir da segunda rodada, ainda vai
        ...
        if(print&&countSuccesses>50) %Se tiver menos que 50 sucessos, resultado é ruim
            date = clock;
            matFileName = sprintf('%d_%d_%d-%dh%dmin.mat',date(1),date(2),date(3),date(4),date(5));
