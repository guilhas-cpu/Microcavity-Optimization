function [xT xqwT xbT cavthickHalf cavthick xcavt] = Sample_3_final_results(s,ncs,nci,N,xqw,xb,c1,c2,c3,c4,cqw,lambdaR,T,teta,shift)
...
n1R = Dispersion(c1,lambdaR,T); % Refractive index of the DBR 1st layer
n2R = Dispersion(c2,lambdaR,T); % Refractive index of the DBR 2nd layer
n3R = Dispersion(c3,lambdaR,T); % Refractive index of the cavity
n4R = Dispersion(c4,lambdaR,T); % Refractive index of the QWs, substrate and cap layer
...
%%%%
if c1 ~= 0 %If the 1st layer have Aluminium in your composition
...
%             fim = 2*ncs+1; %final counter for the upper DBR
%         else %n3R < n2R
%             fim = 2*ncs; %final counter for the upper DBR adding a new layer
%         end
    end
else %c1=0 - GaAs
    ...
    if ncs == 0
        ...
        if n3R < n2R
            fim = 2*ncs+1; %counter for the upper DBR - add one layer
        else %n3R <= n2R
...
if c1 ~= 0 %with caplayer
    for j = beg:fim
        if rem(j,2) == 0
            x(j)= x1; %Thickness of 1st layer at 300K [nm].
            xT(j) = x1T; %New thickness
        else
            x(j)= x2; % Thickness of 2nd layer at 300K [nm].
            xT(j) = x2T; %New thickness
        end
    end
else %without caplayer
    for j = beg:fim
        if rem(j,2) == 0
            x(j)= x2; %Thickness of 1st layer at 300K [nm].
            xT(j) = x2T; %New thickness
        else
            x(j)= x1; % Thickness of 2nd layer at 300K [nm].
            xT(j) = x1T; %New thickness
        end
    end
end
...
xcavt = round(xcavt,1);
    if(print)
        x = round(x,1); %print disabled 
        disp('Microcavity total thickness at 300K [nm]:');
        sum(x)
        xT = round(xT,1); %print disabled
