function z = Reflectance_adapted_2(vars)
    z = zeros(1,7);
    global print;
    global range;
    global plota;
    ...
    if(plota)
        figure()
        plot(lambda,n_1,'k:',lambda,n_2,'b--',lambda,n_3,'b',lambda,n_4,'k',lambda,n_qw,'k--',...
            'LineWidth',1.5)
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    [absorption lamb_gap4] = Absorption(c3,lambdaR,T); %Absorption coefficient[m^-1], Gap wavelength
    if(print)
    ...
    if ncs == 0 %DBR only
        if n1R == n4R 
            ...
            if(print)
                disp('Your microcavity have a caplayer');
            end
        end
    else
        if nci == 0 %DBR only
            ...
            if xT(1) <= 1
                ...
                x2 = xT(2); % DBR 2nd layer
                if(print)
                    disp('Your microcavity dont have a caplayer');
                end
            end
        end
    end
    ...
    linewidth = 1.2;%FWMH at 10K [nm]% (1e9)*h*c*((1.6e-19)*(0.076-0.0426+0.0194-0.0137))/((1.6e-19)*(1.5187+0.051+0.0174))^2%QW linewidth emission [nm]
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %Penetration length of the optical normal mode and effective refractive index
    %n_eff - Toshihiko Baba, et. al., IEEE JOURNAL OF QUANTUM ELECTRONIC, 27, 6 (1991).
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if n1R > n2R
        nH = n1R;
        nL = n2R;
    ...
    Lcav = (s*lambdaR/(2*n3R));
    nr = nH/n3R;
    %neff = (nDBR*(Lp_top + Lp_bottom) + lambdaR)/(Lp_top + Lp_bottom + (lambdaR/n3R))
    neff = n3R*sqrt((Lcav + (nr^2)*(lambdaR/(2*n3R))*2*(p^2)/(1-(p^2)))/(Lcav + (lambdaR/(2*n3R))*(1+(p^2))/(1-(p^2))));
    Dn = sqrt((n1R - n2R)^2);
    kapa = pi*Dn/(2*lambdaR);
    csio = Lcav/(Lcav + (lambdaR/(2*n3R))*(1+(p^2))/(1-(p^2)));
    Vol = N*xqw*1e-9*pi*(40e-6)^2;%volume of the active region
    Co = csio*((lamb_QW*1e-9)^4)/(4*(pi^2)*(neff^3)*Vol*linewidth*1e-9);
    ...
    if(print)
        ...
        kapa = kapa
        disp('The normal mode cavity confinement factor to the defined temperature is:');
        csio = csio
        disp('The spontaneus emission factor for the cavity normal mode to the defined temperature is:');
        Co = Co
    ...
    %The stop band width
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    StopBand =(4/pi)*Resonance*asin((nH - nL)/(nH + nL));
    if(print)
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%DISTANCE BETWEEN RESONANCE AND EMISSION PEAKS%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
